...
const ProfilePage = () => {
  ...
  return (
    <div className="min-h-screen pb-28 bg-background md:pb-28 lg:pb-16 lg:pt-24">
      <div className="mx-auto max-w-lg px-8 pt-10 md:max-w-2xl md:px-10 lg:max-w-3xl lg:px-12">
        <div className="flex items-baseline justify-between mb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => setSection("settings")} className="text-foreground/75 hover:text-foreground/70 transition-colors">
        ...
        <div className="flex items-center gap-5 mb-8">
          <div className="relative">
            <div className="flex h-14 w-14 items-center justify-center rounded-full bg-foreground/[0.03] overflow-hidden">
              {profile?.avatar_url ? (
            ...
            <button
              ...
              className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-accent/20 text-accent/70 hover:bg-accent/30 transition-colors"
            >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <p className="font-display text-[17px] text-foreground/85 truncate">{displayName}</p>
              {profile?.username && (
                <span className="text-[10px] text-foreground/50 truncate">@{profile.username}</span>
              )}
            </div>
            <p className="text-[10px] text-foreground/55 truncate">{user?.email}</p>
          </div>
        </div>

        {/* Section nav — horizontal scrollable */}
        <div className="-mx-2 mb-8 overflow-x-auto scrollbar-hide">
          <div className="flex items-center gap-1 px-2">
            {([
              ...
              { key: "showrooms" as const, label: "SHOWROOMS" },
              { key: "settings" as const, label: "SETTINGS" },
            ]).map(s => (
              <button
                key={s.key}
                onClick={() => setSection(s.key)}
                className={`relative shrink-0 px-3.5 py-2 text-[10px] font-semibold tracking-[0.18em] transition-colors ${
                  section === s.key ? "text-foreground" : "text-foreground/45 hover:text-foreground/70"
                }`}
              >
                {s.label}
                {section === s.key && (
                  <span className="absolute bottom-0 left-1/2 h-px w-6 -translate-x-1/2 rounded-full bg-accent/70" />
                )}
              </button>
            ))}
          </div>
      ...
      <div className="mx-auto max-w-lg px-8 space-y-8 md:max-w-2xl md:px-10 lg:max-w-3xl lg:px-12">
          ...
          <>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                {profile?.bio && <p className="text-[11px] text-foreground/75 italic">{profile.bio}</p>}
                {emailVerified ? (
                  <span className="flex items-center gap-1 text-[10px] text-green-500/70"><CheckCircle className="h-2.5 w-2.5" /> Verified</span>
                ) : (
                  <span className="flex items-center gap-1 text-[10px] text-orange-400/70"><XCircle className="h-2.5 w-2.5" /> Unverified</span>
                )}
                {profile?.location && <span className="text-[10px] text-foreground/70">📍 {profile.location}</span>}
              </div>
              <button onClick={() => setIsEditing(!isEditing)} className="text-foreground/55 hover:text-foreground">
            ...
            {isEditing && (
              <div className="rounded-xl border border-border/20 bg-card/30 p-5 space-y-4">
                ...
                <div>
                  ...
                  {usernameError && <p className="mt-1 text-[10px] text-destructive">{usernameError}</p>}
                </div>
                {[
            ...
            <div className="grid grid-cols-3 gap-3 sm:gap-4">
              {[
                ...
                { key: "saved", icon: Bookmark, label: t("saved"), value: savedCount, onClick: () => setSection("saved") },
            ...
            {/* Style + Body cards */}
            <Accordion type="multiple" defaultValue={["style", "body"]} className="space-y-2">
        ...
        {section === "tryons" && (
          <div className="py-20 text-center space-y-5">
            <Sparkles className="mx-auto h-7 w-7 text-foreground/40" strokeWidth={1.4} />
            <p className="text-[13px] text-foreground/70">Your AI try-on results</p>
            <p className="text-[11px] text-foreground/50 max-w-xs mx-auto leading-relaxed">
              Generate a fit in FIT LAB and your results will appear here.
            </p>
            <button
              onClick={() => navigate("/fit")}
              className="inline-flex items-center gap-2 rounded-full border border-accent/30 bg-accent/10 px-6 py-2.5 text-[11px] font-semibold tracking-[0.18em] text-accent/80 hover:bg-accent/15"
            >
              OPEN FIT LAB
            </button>
          </div>
        ...
        {section === "purchases" && (
          <div className="py-20 text-center space-y-5">
            <ShoppingBag className="mx-auto h-7 w-7 text-foreground/40" strokeWidth={1.4} />
            <p className="text-[13px] text-foreground/70">No purchases yet</p>
            <p className="text-[11px] text-foreground/50 max-w-xs mx-auto leading-relaxed">
              Items you buy through Wardrobe will be tracked here.
            </p>
            <button
        ...
        {section === "showrooms" && (
          <div className="space-y-6">
            ...
            {myOotds.length > 0 && (
              <div>
                <p className="mb-3 text-[10px] font-semibold tracking-[0.2em] text-foreground/55">{myOotds.length} OOTDs · {totalStars} ★</p>
                ...
                <button onClick={() => navigate("/feed")} className="mt-3 text-[10px] font-semibold tracking-[0.15em] text-accent/70 hover:text-accent">{t("profileViewAll")}</button>
              </div>
            )}
          </div>
        )}
        ...
        {section === "settings" && (
          <div className="space-y-6">
            <div className="flex items-center gap-4 rounded-xl border border-border/20 bg-card/30 px-4 py-3">
            ...
            <div className="space-y-1">
              {[
                ...
                <button key={item.label} onClick={item.action} className="flex w-full items-center gap-5 py-4 transition-colors hover:text-foreground">
                  <item.icon className="h-[18px] w-[18px] text-foreground/75" strokeWidth={1.5} />
                  <span className="flex-1 text-left text-[13px] text-foreground/70">{item.label}</span>
