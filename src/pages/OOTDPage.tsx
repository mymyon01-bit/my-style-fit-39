      ...
      <div className="mx-auto max-w-lg px-6">
        {!user && (
          <p className="pb-4 text-center text-[11px] text-foreground/25">
            <button onClick={() => navigate("/auth")} className="text-foreground/40 underline underline-offset-2">Sign in</button>
            ...
            return (
              <div key={post.id} className="group">
                <div className="relative overflow-hidden rounded-xl">
                  <img src={post.image} alt={post.caption} className="aspect-[3/4] w-full object-cover" loading="lazy" />
                  <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/50 to-transparent p-4 pt-20">
                    <div className="flex items-end justify-between">
                      <div className="flex items-center gap-2">
                        <img src={post.avatar} alt={post.creator} className="h-7 w-7 rounded-full border border-white/20 object-cover" />
                        <span className="text-[11px] font-medium text-white/80">{post.creator}</span>
                      </div>
                      <AuthGate action="give stars">
                        <button onClick={() => handleStar(post.id)} disabled={starsLeft <= 0 && !isStarred}
                ...
                {post.caption && <p className="mt-2 text-[11px] font-light text-foreground/40">{post.caption}</p>}
              </div>
            );
          })}
        </div>
