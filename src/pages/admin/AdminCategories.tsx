    ...
    <div>
      <button
        ...
        className={`flex w-full items-center gap-2 py-2.5 px-3 rounded-lg text-left transition-colors hover:bg-foreground/[0.03] ${
          depth === 0 ? "text-foreground/70 font-medium" : "text-foreground/70"
        }`}
      ...
      >
        ...
        <span className="text-[10px] text-foreground/70 ml-auto">{cat.slug}</span>
      </button>
      {open && hasChildren && (
        <div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] text-foreground/75">Manage via database</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center space-y-3">
          <FolderTree className="mx-auto h-8 w-8 text-foreground/70" />
          <p className="text-[13px] text-foreground/70">No categories found</p>
        </div>
      ) : (
