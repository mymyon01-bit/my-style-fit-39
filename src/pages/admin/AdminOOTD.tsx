  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] text-foreground/40">{posts.length} posts</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center space-y-3">
          <Camera className="mx-auto h-8 w-8 text-foreground/20" />
          <p className="text-[13px] text-foreground/50">No OOTD posts yet</p>
        </div>
      ) : (
          ...
          {posts.map(post => (
            <div key={post.id} className="rounded-xl border border-border/20 bg-card/30 overflow-hidden">
              ...
              <div className="p-3 space-y-1">
                <p className="text-[11px] text-foreground/60 truncate">{post.caption || "No caption"}</p>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-foreground/30">⭐ {post.star_count || 0}</span>
                  <span className="text-[10px] text-foreground/30">{new Date(post.created_at).toLocaleDateString()}</span>
                </div>
