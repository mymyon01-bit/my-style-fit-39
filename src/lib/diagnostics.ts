...
async function flush() {
  ...
  try {
    ...
    // diagnostics must never break the app. The generated `Json` type for
    // metadata is recursive and conflicts with `Record<string, unknown>`,
    // so we cast at this single boundary.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await supabase.from("diagnostics_events").insert(batch as any);
  } catch (err) {
