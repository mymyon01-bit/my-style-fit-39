...
import stormVideo from "../../../public/bg-videos/storm.mp4.asset.json";
import snowVideo from "../../../public/bg-videos/snow.mp4.asset.json";
import sunnyVideo from "../../../public/bg-videos/sunny.mp4.asset.json";
import sunsetVideo from "../../../public/bg-videos/sunset.mp4.asset.json";
import starsVideo from "../../../public/bg-videos/stars.mp4.asset.json";
// Fashion-mood live scenes generated for the Showroom + OOTD background picker.
import runwayVideo from "../../../public/bg-videos/runway.mp4.asset.json";
import neonCityVideo from "../../../public/bg-videos/neon-city.mp4.asset.json";
import desertVideo from "../../../public/bg-videos/desert.mp4.asset.json";
import cafeVideo from "../../../public/bg-videos/cafe.mp4.asset.json";
import metropolisVideo from "../../../public/bg-videos/metropolis.mp4.asset.json";
import rooftopVideo from "../../../public/bg-videos/rooftop.mp4.asset.json";
import libraryVideo from "../../../public/bg-videos/library.mp4.asset.json";
import subwayVideo from "../../../public/bg-videos/subway.mp4.asset.json";
import forestVideo from "../../../public/bg-videos/forest.mp4.asset.json";
import coastVideo from "../../../public/bg-videos/coast.mp4.asset.json";
import studioVideo from "../../../public/bg-videos/studio.mp4.asset.json";

/**
 * Cinematic real-footage videos generated by AI for each theme. When
 * `realistic` mode is on (toggled in MyBackgroundPicker, persisted to
 * localStorage), we render a looping muted video instead of the SVG scene.
 */
const VIDEO_BY_THEME: Partial<Record<OOTDBgTheme, string>> = {
  sakura: sakuraVideo.url,
  leaves: leavesVideo.url,
  rain:   rainVideo.url,
  storm:  stormVideo.url,
  sunny:  sunnyVideo.url,
  stars:  starsVideo.url,
  // Bonus weather scenes — also have SVG fallbacks below.
  snow:   snowVideo.url,
  sunset: sunsetVideo.url,
  // Fashion-mood live-only scenes (no SVG fallback — picker forces realistic).
  runway:     runwayVideo.url,
  neon:       neonCityVideo.url,
  desert:     desertVideo.url,
  cafe:       cafeVideo.url,
  metropolis: metropolisVideo.url,
  rooftop:    rooftopVideo.url,
  library:    libraryVideo.url,
  subway:     subwayVideo.url,
  forest:     forestVideo.url,
  coast:      coastVideo.url,
  studio:     studioVideo.url,
};

const REALISTIC_KEY = "ootd-bg-realistic";

export function loadOOTDBgRealistic(): boolean {
  if (typeof window === "undefined") return true;
  const v = localStorage.getItem(REALISTIC_KEY);
  // Default ON — realistic mode is the new headline experience.
  if (v === null) return true;
  return v === "1";
}

export function saveOOTDBgRealistic(on: boolean) {
  try { localStorage.setItem(REALISTIC_KEY, on ? "1" : "0"); } catch {}
  try { window.dispatchEvent(new CustomEvent("ootd-bg-realistic-change", { detail: on })); } catch {}
}
...
export const OOTD_BG_THEMES: { id: OOTDBgTheme; label: string; emoji: string; description: string }[] = [
  ...
  { id: "rooftop",    label: "Rooftop pool",         emoji: "🏙️", description: "Golden-hour infinity pool over the city" },
  { id: "library",    label: "Old library",          emoji: "📚", description: "Warm lamps, dust in the window light" },
  { id: "subway",     label: "Tokyo subway",         emoji: "🚇", description: "Neon streaks past the train window" },
  { id: "forest",     label: "Misty forest",         emoji: "🌲", description: "Pine fog with sunbeams at dawn" },
  { id: "coast",      label: "Mediterranean coast",  emoji: "🏖️", description: "Turquoise sea against white cliffs" },
  { id: "studio",     label: "Photo studio",         emoji: "📸", description: "White seamless backdrop, soft strobes" },
];
...
export default function OOTDBackground({ theme, realistic = true, contained = false }: Props) {
    ...
    return (
      <div className={`pointer-events-none ${posClass} overflow-hidden`}>
        <div
          ...
          style={{
            background:
              "linear-gradient(180deg, #d3dde9 0%, #e4ebf3 55%, #f1f5f9 100%)",
          }}
        />
        <SoftCloud top="6%"  left="-10%" size={340} delay={0}  duration={200} opacity={0.7} tint="#cfd6e0" />
        <SoftCloud top="14%" left="40%"  size={400} delay={30} duration={220} opacity={0.62} tint="#c2cad6" />
        <SoftCloud top="22%" left="75%"  size={320} delay={15} duration={190} opacity={0.7} tint="#d6dde7" />
          ...
          return (
            <span
              ...
              style={{
                ...
                filter: "drop-shadow(0 1px 2px rgba(150,165,185,0.4))",
                animation: `ootd-snow-fall ${dur}s linear ${delay}s infinite`,
              }}
            >
              ❄
            </span>
    ...
    return (
      <div className={`pointer-events-none ${posClass} overflow-hidden`}>
        <div
          ...
          style={{
            background:
              "linear-gradient(180deg, #1a1238 0%, #4a2752 22%, #c84a5a 52%, #f29365 75%, #f5c98a 100%)",
          }}
        />
        ...
        <div
          ...
          style={{
            ...
            marginLeft: "-120px",
            borderRadius: "50%",
            background: "radial-gradient(circle, #fff2c8 0%, #ffd58a 40%, rgba(255,170,90,0) 75%)",
            filter: "blur(2px)",
            animation: "ootd-sun-pulse 6s ease-in-out infinite",
          }}
        />
        {/* Long horizontal light beams across the horizon */}
        <div
        ...
        />
        <SoftCloud top="22%" left="-12%" size={360} delay={0}  duration={220} opacity={0.55} tint="#ffb680" />
        <SoftCloud top="32%" left="50%"  size={420} delay={35} duration={240} opacity={0.5}  tint="#ff9a6a" />
        <SoftCloud top="42%" left="80%"  size={300} delay={15} duration={200} opacity={0.6}  tint="#ffc88c" />
        <style>{`
          @keyframes ootd-sun-pulse {
            0%, 100% { transform: scale(1); filter: blur(2px) brightness(1); }
            50% { transform: scale(1.06); filter: blur(2px) brightness(1.12); }
          }
          @keyframes ootd-cloud-drift {
            from { transform: translateX(0); }
            to   { transform: translateX(calc(110vw + 600px)); }
  ...
  // ── Fashion-mood scenes (SVG fallback when realistic mode is off) ──────
  // These themes are designed for live video; the SVG fallback is a tasteful
  // gradient + accent so the background never looks broken if realistic OFF.
  if (theme === "runway" || theme === "neon" || theme === "desert" || theme === "cafe" || theme === "metropolis") {
    const presets: Record<string, { bg: string; accent: string }> = {
      runway:     { bg: "linear-gradient(180deg, #0a0a0d 0%, #1a1620 60%, #2a1c2c 100%)", accent: "radial-gradient(ellipse at 50% 90%, rgba(255,220,180,0.25), transparent 55%)" },
      neon:       { bg: "linear-gradient(180deg, #0d0820 0%, #1a0f2e 50%, #2a1238 100%)", accent: "radial-gradient(circle at 30% 70%, rgba(236,72,153,0.3), transparent 45%), radial-gradient(circle at 75% 40%, rgba(34,211,238,0.28), transparent 45%)" },
      desert:     { bg: "linear-gradient(180deg, #f5d59a 0%, #e9a86a 55%, #b8703a 100%)", accent: "radial-gradient(ellipse at 50% 100%, rgba(120,60,20,0.35), transparent 60%)" },
      cafe:       { bg: "linear-gradient(180deg, #f1e4d0 0%, #e6d0b3 55%, #c9a982 100%)", accent: "radial-gradient(circle at 80% 20%, rgba(255,240,210,0.55), transparent 50%)" },
      metropolis: { bg: "linear-gradient(180deg, #1a2540 0%, #2a3a5e 45%, #d68a9a 100%)", accent: "radial-gradient(ellipse at 50% 80%, rgba(255,180,160,0.25), transparent 60%)" },
    };
