...
const PieceRow = ({ piece, label }: { piece: OutfitPiece; label: string }) => (
  <div className="flex items-center gap-4 py-3 md:py-3.5">
    ...
    <div className="flex-1 min-w-0">
      <p className="text-[13px] text-foreground/75 truncate md:text-[14px]">{piece.name}</p>
    </div>
    <p className="text-[10px] text-foreground/65 capitalize md:text-[11px]">{label}</p>
  </div>
);
...
const DailyPicks = () => {
  ...
  if (loading) return <div className="flex items-center justify-center py-20"><Loader2 className="h-4 w-4 animate-spin text-foreground/80" /><span className="ml-3 text-[11px] text-foreground/75">Curating…</span></div>;
  if (error) return <div className="py-14 text-center"><p className="text-[11px] text-foreground/65">{error}</p><button onClick={fetchDaily} className="mt-2 text-[10px] text-accent/70 hover:text-accent">Retry</button></div>;
  ...
  return (
    <div className="space-y-7 md:space-y-8">
      ...
      <div className="flex gap-6 md:gap-8">
        {outfits.map((o, i) => (
          <button key={i} onClick={() => setActiveIndex(i)} className={`text-[12px] font-light transition-colors duration-300 md:text-[13px] ${i === activeIndex ? "text-foreground/80" : "text-foreground/75"}`}>
            {o.label || `Look ${i + 1}`}
          </button>
        ))}
      </div>
