    ...
    <div ref={ref} className={`relative ${className}`}>
      ...
      {open && (
        <div className="absolute right-0 top-10 z-50 flex flex-col gap-1 rounded-xl border border-border/30 bg-card/95 backdrop-blur-xl p-2 shadow-elevated animate-fade-up min-w-[140px]">
          <button onClick={copy} className="hover-burgundy flex items-center gap-2.5 rounded-lg px-3 py-2 text-[11px] font-medium text-foreground/75">
            <Link2 className="h-3.5 w-3.5" /> Copy link
          </button>
          <button onClick={() => openLink("https://wa.me/?text=")} className="hover-burgundy flex items-center gap-2.5 rounded-lg px-3 py-2 text-[11px] font-medium text-foreground/75">
            <MessageCircle className="h-3.5 w-3.5" /> WhatsApp
          </button>
          <button onClick={() => openLink("https://t.me/share/url?url=")} className="hover-burgundy flex items-center gap-2.5 rounded-lg px-3 py-2 text-[11px] font-medium text-foreground/75">
